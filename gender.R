...
#use random forest to predict the missing value in PrimaryBrainRegion
rf.model <- randomForest(factor(PrimaryBrainRegion)~.,data=na.omit(dataset))
id_na <- which(is.na(dataset$PrimaryBrainRegion))
...
X <- dataset[training_id,-1]




# the label variable 10g
Y <- dataset[training_id,1]
...
#test data 
X_test <- dataset[-training_id,-1] 
Y_test <- dataset[-training_id,1]
